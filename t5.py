...
CONFIG = [
    dict(game="SpaceInvaders-ramDeterministic-v0",
         n_feature=128, n_action=6, continuous_a=[False], ep_max_step=2000, eval_threshold=50000),
...
def get_reward(shapes, params, env, ep_max_step, continuous_a, seed_and_id=None,render=False):
    ...
    for step in range(ep_max_step):
        ...
        if r > 0: r = 1
        lives_ = info['ale.lives']
        if lives_ < 3: done = True
...
def build_net():
    ...
    s0, p0 = linear(CONFIG['n_feature'], L1)
    s1, p1 = linear(L1, L2)
    s2, p2 = linear(L2, CONFIG['n_action'])
...
if __name__ == "__main__":
    ...
    for g in range(N_GENERATION):
        t0 = time.time()
        net_params, gradients,kid_rewards = train(net_shapes, net_params, optimizer, utility, pool)

        # test trained net without noise
        net_r = get_reward(net_shapes, net_params, env, CONFIG['ep_max_step'], CONFIG['continuous_a'], None,render=True)
        mar = net_r if mar is None else 0.9 * mar + 0.1 * net_r       # moving average reward
        print(
            'Gen: ', g,
            '| Net_R: %.1f' % mar,
