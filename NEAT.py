...
class ESModel():
    def __init__(self):
        ...
        model.add(Convolution2D(20,(7,7),strides=(3,3),padding='same',input_shape=(140,160,1),name='1'))
        model.add(Activation('tanh'))

        model.add(Convolution2D(20,(5,5),strides=(3,3),padding='same',name='2'))
        model.add(Activation('tanh'))

        model.add(Convolution2D(30,(4,4),strides=(2,2),padding='same',name='3'))
        model.add(Activation('tanh'))
        
        model.add(Convolution2D(30,(4,4),strides=(2,2),padding='same',name='4'))
        model.add(Activation('tanh'))

        model.add(Convolution2D(50,(3,3),strides=(1,1),padding='same',name='5'))
        model.add(Activation('tanh'))

        model.add(Flatten())
            
        model.add(Dense(32,name='6'))
        model.add(Activation('tanh'))
        model.add(Dense(3,name='7'))
        ...
        for layer in model.layers:
            if layer.name.isnumeric():  
                a = model.get_layer(name=layer.name)
                w = a.get_weights()
                if len(w) > 0:
